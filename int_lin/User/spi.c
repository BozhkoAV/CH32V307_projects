...
void spi_init(void)
{
	//Enable clock for PORTA, SPI1 and DMA1
	RCC->APB2PCENR |=  RCC_IOPAEN | RCC_SPI1EN;
	RCC->AHBPCENR |= RCC_DMA1EN;
	
	//Enable PC7 PA6(MISO) - Floating input, PC6 PA7(MOSI) and PC5 PA5(SCK) - Multiplexed push pull output
	//GPIO max speed - 50Mhz
	GPIOA->CFGLR = (GPIOA->CFGLR & ~(GPIO_CFGLR_CNF6 | GPIO_CFGLR_CNF7 | GPIO_CFGLR_CNF5 \
					| GPIO_CFGLR_MODE7 | GPIO_CFGLR_MODE5)) \
					| GPIO_CFGLR_CNF6_0 | GPIO_CFGLR_CNF7_1 | GPIO_CFGLR_CNF5_1 \
					| GPIO_CFGLR_MODE7_0 | GPIO_CFGLR_MODE7_1 | GPIO_CFGLR_MODE5_0 | GPIO_CFGLR_MODE5_1;
	
	//Set SPI1, max clock 48Mhz/2 = 24Mhz, master mode, full-duplex mode,8bit data length
	//Internal slave select and software slave management
	...
	//No memory or peripheral increment
